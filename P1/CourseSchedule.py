...
class CourseSchedule:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        preToCourses: Dict[int, List[int]] = dict()
        for course in range(0, numCourses):
            preToCourses[course] = list()

        indegrees = [0 for _ in range(0, numCourses)]
        for coursePrePair in prerequisites:
            course = coursePrePair[0]
            pre = coursePrePair[1]
            preToCourses.get(pre).append(course)
            indegrees[course] += 1
        ...
        while queue:
            pre = queue.popleft()
            count += 1
            next_courses = preToCourses.get(pre)
