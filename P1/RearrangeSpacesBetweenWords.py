...
# that number is maximized. If you cannot redistribute all the spaces equally, place the extra spaces at the end,
...
class RearrangeSpacesBetweenWords:
    ...
    def reorderSpaces(self, text: str) -> str:
        ...
        words = text.split()  # python string split will trim spaces
        word_count = len(words)
        space_count = text.count(self.SPACE)
        if word_count <= 1:
            return words[0] + self.SPACE * space_count

        space_chunk_count = word_count - 1
        space_count_per_chunk = space_count // space_chunk_count
        extra_space_count = space_count % space_chunk_count
        words = (self.SPACE * space_count_per_chunk).join(words)

        return words + extra_space_count * self.SPACE
